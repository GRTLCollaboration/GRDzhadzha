...
#include "CCZ4ComplexScalarField.hpp"
...
int main()
{
    ...
    for (int ires = 0; ires < num_resolutions; ires++)
    {
        ...
        BoostedBH boosted_bh(bg_params, dx);
        BoxLoops::loop(
            AssignFixedBGtoBSSNVars<BoostedBH>(boosted_bh, dx, center_vector),
        ...
        CCZ4ComplexScalarField<ComplexPotential> scalar_field(potential);

        BoxLoops::loop(MatterCCZ4RHS<CCZ4ComplexScalarField<ComplexPotential>>(
                           scalar_field, ccz4_params, dx, sigma,
                           CCZ4RHS<>::USE_BSSN, G_Newton),
                       fixedbg_fab, rhs_fab);

        // Calculate the Matter RHS using the analytic derivatives
        ComplexScalarField<ComplexPotential> fixed_scalar_field(potential);
        MatterEvolution<ComplexScalarField<ComplexPotential>, BoostedBH>
        ...
        // values
        BoxLoops::loop(
            ExcisionTest<ComplexScalarField<ComplexPotential>, BoostedBH>(
