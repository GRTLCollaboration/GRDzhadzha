...
#include "BoostedBHComplexProcaLevel.hpp"
...
{
    CH_TIME("BoostedBHComplexProcaLevel::initialData");
    if (m_verbosity)
        pout() << "BoostedBHComplexProcaLevel::initialData " << m_level << endl;
    ...
    InitialProcaData initial_pf(
        m_p.proca_amplitude, m_p.proca_mass, m_p.center, m_p.bg_params, m_dx,
...
void BoostedBHComplexProcaLevel::specificPostTimeStep()
{
    ...
    {
        fillAllGhosts();
        ComplexProcaField proca_field(m_p.proca_mass, m_p.proca_damping);
        ProcaGaussDiagnostic gauss_constraint{};
        BoostedBH boosted_bh(m_p.bg_params, m_dx);
        EnergyConservation<ComplexProcaField, BoostedBH> energies(
        ...
        BoxLoops::loop(
            ExcisionDiagnostics<ComplexProcaField, BoostedBH>(
                                                 ...
                                                 const double a_time)
{
    ...
    ComplexProcaField proca_field(m_p.proca_mass, m_p.proca_damping);
    BoostedBH boosted_bh(m_p.bg_params, m_dx);
    MatterEvolution<ComplexProcaField, BoostedBH> my_evolution(
...
void BoostedBHComplexProcaLevel::computeTaggingCriterion(
